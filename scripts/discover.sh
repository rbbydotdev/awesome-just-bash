...
# Capture actual LLM cost from llm's log DB (OpenRouter returns real billed cost in token_details)
LLM_USAGE=$(llm logs -n 1 --json 2>/dev/null | jq -r '.[0] | {
  model: .model,
  input: (.input_tokens // 0),
  output: (.output_tokens // 0),
  cost: ((.token_details // "{}") | fromjson | .cost // null)
}' 2>/dev/null || echo '{}')
log "usage: $LLM_USAGE"
...
# Build PR body with checkboxes
{
  ...
  COST_LINE=$(echo "$LLM_USAGE" | jq -r '
    if .cost != null then
      "LLM cost: $\(.cost | tostring | .[0:6]) — \(.model), \(.input) in / \(.output) out tokens."
    elif .input > 0 then
      "LLM usage: \(.model), \(.input) in / \(.output) out tokens."
    else empty end
  ' 2>/dev/null || true)
